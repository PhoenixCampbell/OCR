const ocrDemo = (() => {
	...
	function flashSuccess() {
		...
		if (!el) return;
		el.classList.add("on");
		setTimeout(() => el.classList.remove("on"), 1000); // 1s flash
	}

	// Downscale to 20x20 grayscale
	function get20x20() {
		...
		for (let i = 0; i < data.length; i += 4) {
			const r = data[i],
				g = data[i + 1],
				b = data[i + 2];
			// grayscale and invert so black strokes are about 1.0
	...
	async function train() {
		...
		try {
			await fetch("http://127.0.0.1:8000/ocr", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					train: true,
					trainArray: [{ y0: pixels, label: Number(label) }],
				}),
			});
			// Success feedback: soft green flash for 1s
			flashSuccess();
		} catch (e) {
			console.error("Train failed:", e);
			// optional: show a small inline error instead of alert
		}
	}
